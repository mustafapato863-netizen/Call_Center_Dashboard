...
export function usePerformanceData(month: MonthKey, location: LocationKey) {
  return useMemo(() => {
    ...
    const prevMonthName = currMonthName === 'March' ? 'February' : currMonthName === 'February' ? 'January' : null;
    ...
    if (prevMonthName) {
      ...
      if (currSum && prevSum) {
        // Attendance
        const currAttend = currSum.avgAttendRate * 100;
        const prevAttend = prevSum.avgAttendRate * 100;
        const attendDiff = currAttend - prevAttend;
        ...
        const currAgents = allData.filter((r) => r.identity.month === currMonthName && r.identity.name.toLowerCase() !== 'total');
        const prevAgents = allData.filter((r) => r.identity.month === prevMonthName && r.identity.name.toLowerCase() !== 'total');
        ...
        const currScore = currAgents.length > 0 ? currAgents.reduce((s, a) => s + a.evaluation.score, 0) / currAgents.length : 0;
        const prevScore = prevAgents.length > 0 ? prevAgents.reduce((s, a) => s + a.evaluation.score, 0) / prevAgents.length : 0;
        const scoreDiff = (currScore - prevScore) * 100;
